<!DOCTYPE html>
<html>

<head>
  <link rel="stylesheet" href="css/main.css">
</head>

<body>
<header>
<div class="header">
  <a href="index.php" class="logo">GetYourCar</a>

    <div class="header-right">
    <a class="active" href="#index.php">Home</a>
    <a href="cars.php">Cars</a>
    <a href="contact.php">Contact</a>
    <a href="login.php">Log In</a>
    <a href="signup.php">Sign Up</a>  
    <a href="about.php">About Us</a>  
    <a href="logout.php">Logout</a>                      
  </div>

</div>
</header>

<div class="home-image">
  <img src="img/home1.jpg" style="width:100%;">
</div>

<div class="features">
  <div class="container">

    <div class="four features-column">
      <h2>Wide Range of Brand</h2>
      <p>In our fleet we have 100 premium cars, micro bus, pick up, well as truck, covered van and commercial vehicles.</p>
    </div>

    <div class="four features-column">
      <h2>Receive What matters</h2>
      <p>We use all our experience of our customers to bring you the car you need and the quality of service you want.</p>
    </div>

    <div class="four features-column">
      <h2>Ensure Security</h2>
      <p>We always try to ensure top security for the passengers. For ensuring security GetYourCar always connected to you.</p>
    </div>

    <div class="four features-column">
      <h2>100% Responsive</h2>
      <p>GetYourCar driver will provide you the faithful support. They always have beem committed to you.</p>
    </div>
  </div>
</div>

<div class="home-image">
  <img src="img/home2.png" style="width:100%;">
</div>

<br>
<br>

<div class="container">
    <div class="cars-row">

      <h1>Why Choose Us</h1>

      <div class="cars-column">
        <img src="img/service1.png">
        <div class="banner-subheading">
          <h5">AFFORDABLE SERVICE CHARGES</h5>
          <p>Clients can tailor his travelling needs through our wide area of service policy. There are no hidden charges or Extra Tariff payment after execution.</p>
        </div>
      </div>

      <div class="cars-column">
        <img src="img/service2.png">
        <div class="banner-subheading">
          <h5">LARGE FLEET OF CARS</h5>
          <p>To meet up Clients social call or travel to utmost urgency, we can offer our perfect solution for the vehicle clients may need for their purposes.</p>
        </div>
      </div>

      <div class="cars-column">
        <img src="img/service3.png">
        <div class="banner-subheading">
          <h5">DEDICATION & COMMITMENT</h5>
          <p>Our drivers are abided by every needs of the clients. A client starts our services by getting inside of a vehicle, we finish.</p>
        </div>
      </div>

    </div>
</div>

<footer>
  <div class="footer">
    &copy; Copyright 2019 GetYourcar
  </div>
</footer>

</body>
</html>